            ...
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-sm-12 col-md-8">
                        <form action="{{ url('adminboard/'.$post->id) }}"  method="POST" id="form-task" enctype="multipart/form-data">
                            @csrf
                            @method("PATCH")
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select @error('subcategory_id') is-invalid @enderror" name="subcategory_id" id="post-status">
                                                ...
                                                @foreach($subcategories as $item)
                                                    <option value="{{$item->id}}" {{ (old('subcategory_id') ?? $post->subcategory_id) == $item->id ? 'selected' : '' }}> {{$item->subcategory}} </option>
                                                @endforeach
                                            </select>
